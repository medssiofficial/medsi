...
export const updateDoctorOnboardingExpertises = async (
	expertises: string[],
): Promise<DoctorMe> => {
	const response = await fetch(API_ROUTES.DOCTOR.ONBOARDING.EXPERTISES.path, {
		method: API_ROUTES.DOCTOR.ONBOARDING.EXPERTISES.method,
		headers: {
...
export const useUpdateDoctorOnboardingExpertises = () => {
	const queryClient = useQueryClient();

	return useMutation({
		mutationKey: [API_ROUTES.DOCTOR.ONBOARDING.EXPERTISES.key],
		mutationFn: updateDoctorOnboardingExpertises,
		onSuccess: (doctor) => {
